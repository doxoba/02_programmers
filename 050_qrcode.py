...
# `code`의 각 인덱스를 `q`로 나누었을 때 
# 나머지가 `r`인 위치의 문자를 앞에서부터 순서대로 이어 붙인 문자열을 
...
# [입출력 예 설명]
# 입출력 예 #1
# - 예제 1번의 `q`와 `r`은 각각 3, 1이고 
# 인덱스와 그 값을 `q`로 나눈 나머지가 잘 보이도록 표로 만들면 다음과 같습니다.
...
# 입력: q, r, code
# 출력: 
# 어떻게: code의 각 인덱스를 q로 나눴을 때 나머지가 r인 위치의 문자를 앞에서부터 순서대로 이어붙인 문자열
...
def solution(q,r,code):
    answer = ''
    for i in range(len(code)): # 인덱스 숫자를 세야되니까 range(len()) 사용
        if i % q == r:
            answer += code[i]
    return answer
